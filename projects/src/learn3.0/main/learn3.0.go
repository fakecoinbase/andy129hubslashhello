...
func calFunc(){
	...
	fmt.Printf("浮点(负数)除以 浮点(负数)： %v\n", -5.0/-2.0)  // "2.5"， 编译正确
	fmt.Printf("浮点(正数)除以 浮点(负数)： %v\n", 5.0/-2.0)   // "-2.5"， 编译正确
	fmt.Printf("整数(正数)除以 浮点(负数)： %v\n", 5/-2.0)     // "-2.5"， 编译正确
	fmt.Printf("浮点(负数)除以 整数(正数)： %v\n", -5.0/2)     // "-2.5"， 编译正确
	fmt.Printf("整数(负数)除以 整数(正数)： %v\n", -5/2)       // "-2"， 编译正确
	fmt.Printf("整数(正数)除以 整数(正数)： %v\n", 5/2)        // "2"， 编译正确

	/*	总结： 在 Go语言中
		取模运算符 %,
			1，仅能用于整数，不能用于 浮点数。
			2，取模运算结果的 正负号 总是与 被除数一致： 例： -5%3 = -2 ,  -5%-3= -2
		除法运算符 /，
			1，可应用于 整数，浮点数和复数
			2，取模运算结果是否为整型，取决于操作数是否都为整型，整数相除，商会舍弃小数部分，如果有一个为浮点，则结果为浮点
					例：5.0/4.0= 1.25,   5/4= 1,   5/4.0= 1.25，  5.0/-2.0=-2.5,  -5/2=-2
			3, 取模运算结果的 正负号，与 正常算术规则一致。
					有一个为负，则结果为负；     例：5.0/-2.0= -2.5
					两个都为负，则结果为正；     例：-5.0/-2.0= 2.5
					两个都为正，则结果为正。     例：5/2= 2
	 */
}
